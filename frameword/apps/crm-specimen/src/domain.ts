...
export const DASHBOARDS: Dashboard[] = [
  {
    ...
    // two SpaceGroups: the sidebar renders one mono section label per group —
    // the legacy grouped-nav pattern (INVESTING / OPERATIONS) speaks Stax
    groups: [{
      label: "The framework",
      spaces: [
...
export const DOMAIN: Record<string, DomainNode> = {
  ...
  "sec:model": {
    ...
    blocks: [
      ...
      { kind: "row", label: "WIDTH", text: "From the registry only: S 380 · M 480 · L 640 · XL 800 · XXL fills the remaining stage. A device-local override lives in the foot gear; shared state never stores a width." },
    ],
  ...
  "md:sizes": {
    panelType: "doc", title: "Sizing grammar: S · M · L · XL · XXL", subtitle: "Width belongs to the kind, not the user. XXL is fluid: it takes every pixel the stage has left.",
    blocks: [
  ...
  "imp:keys": {
    ...
    blocks: [
      { kind: "card", label: "Live: the real map", text: "⌘K palette (every space, panel and action; ↑↓ moves, ↵ opens, esc closes) · ⌘B toggles the sidebar · ⌘J toggles the agent drawer · ⌘1-3 switches organization · P pins/unpins the focused panel · ctrl+X closes the focused panel · ⌘Z / ⇧⌘Z undo-redo the last workspace move (a focused canvas keeps its own history) · esc closes in precedence order: palette → drawer → menus → leaf panel. On the canvas: ⌘Z / ⇧⌘Z undo-redo, ⌘D duplicates, ⌘A selects all, arrow keys nudge, ⌫ deletes the selection." },
...
DOMAIN["gl:state"] = {
  ...
  blocks: [
    ...
    { kind: "row", label: "REGISTRY", text: "panelType → width class. S 380 · M 480 · L 640 · XL 800 · XXL fluid (fills the remaining stage). Width belongs to the KIND, never to the user or the content (Law 7)." },
  ],
};
DOMAIN["gl:anatomy"] = {
